...
class FundConvertAll extends StatefulWidget {
  const FundConvertAll({super.key});

  // const FundConvertAll({super.key});
  @override
  // ignore: library_private_types_in_public_api
  _FundConvertAllState createState() => _FundConvertAllState();
}

class _FundConvertAllState extends State<FundConvertAll> {
  ...
  String? dropdownValueRight;
  double usdtAmount = 0;
  var currencyName = "Token";
  var currencyNameRight = "Token";
  var currencyRate = "0";
  var currencyRateRight = "0";
  var liveRate;
  ...
  void getUsdtRate() {
    ...
    if (currencyRate.isNotEmpty &&
        currencyRateRight.isNotEmpty &&
        tokenController.text.isNotEmpty) {
      rate = double.parse(currencyRate.toString());
      double tkn = double.parse(currencyRate.toString()) *
          double.parse(tokenController.text);
      setState(() {
        usdtAmount = tkn / double.parse(currencyRateRight.toString());
      });
    } else {
      setState(() {
        usdtAmount = 0.00;
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                                ...
                                items: [
                                  ...
                                  for (var rowData in dropdownData)
                                    DropdownMenuItem(
                                      ...
                                      onTap: () {
                                        currencyNameRight = rowData['name'];
                                        currencyRateRight = rowData['price'];
                                      },
                                    )
                                ],
                                onChanged: (String? newValue) {
                        ...
                        const SizedBox(height: 20),
                        Text(
                          "$currencyNameRight Amount : $usdtAmount",
  ...
  Future<void> Convert() async {
    ...
    print(APIPaths.fundConvertAll);
    print(dropdownValue);
    if (_formKey.currentState != null && _formKey.currentState!.validate()) {
      setState(() {
        isLoading = true;
      });
      var url = Uri.parse(APIPaths.fundConvertAll);
      var body = {
        'u_id': userId,
        'from_wallet': dropdownValue,
        'to_wallet': dropdownValueRight,
